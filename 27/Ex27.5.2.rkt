...
;;purpose : search for a row in lsol whose first coefficient is not 0,
;;(remove
(define (switchrow lsol)
  (cond
    [(empty? lsol) empty]
    [else (cons (subtract-not-zero lsol)  (remove (subtract-not-zero lsol) lsol))]))


;;subtract-not-zero:(list (list 0 1 2) (list 0 2 2) (list 1 2 2)) -> (list (list 1 2 2))
(define (subtract-not-zero lsol)
  (cond
    [(empty? lsol) (error 'error)]
    [(= (first (first lsol)) 0) (subtract-not-zero (rest lsol))]
    [else (first lsol)]))
